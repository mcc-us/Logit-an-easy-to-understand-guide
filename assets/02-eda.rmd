...
```{r 21, message=FALSE}
# Import libraries
Libraries <- c("knitr", "readr", "RColorBrewer", "corrplot")
for (i in Libraries) {
...
# Import RAW data
df.logit <- read.csv("~/Dropbox/01-Logit-paper/assets/datasets-logit.csv")
```
...
```{r 27, echo=TRUE}
is.data.frame(df.logit)
class(df.logit$Purch)
class(df.logit$prodA)
class(df.logit$prodB)
class(df.logit$prodC)
class(df.logit$prodD)
class(df.logit$prodE)
```
...
plot(AA_ave,
     ...
     type = "b",
     xaxt = "n")
axis(1, at = 1:20, labels = names(df.logit[, 2,4-15]))
```
...
# A-1
ctrl_set <- df.logit[which(df.logit$Purch == "0"), ]
buy_set <- df.logit[which(df.logit$Purch == "1"), ]
